<template>
    <li v-if="!forlder.hasOwnProperty('request')">
        <a @click="parseRequests(forlder, 'dropdown-'+forlder.name+'-'+uniqueIdentifier)" class="flex items-center cursor-pointer p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-200 dark:hover:bg-gray-700">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 12.75V12A2.25 2.25 0 014.5 9.75h15A2.25 2.25 0 0121.75 12v.75m-8.69-6.44l-2.12-2.12a1.5 1.5 0 00-1.061-.44H4.5A2.25 2.25 0 002.25 6v12a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18V9a2.25 2.25 0 00-2.25-2.25h-5.379a1.5 1.5 0 01-1.06-.44z" />
            </svg>
            <span class="flex-1 ml-3 whitespace-nowrap" style="display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical; overflow: hidden !important;">{{ forlder.name }}</span>
        ...
        <ul v-if="forlder?.item" :id="'dropdown-'+forlder.name+'-'+uniqueIdentifier" class="hidden">
            <render-forlder v-for="nextItem in forlder.item" :key="nextItem.name+Math.random().toString(16).slice(2)" :forlder="nextItem" />
        </ul>
    </li>
</template>
<script>
