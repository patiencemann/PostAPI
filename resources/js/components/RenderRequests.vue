<template>
    <aside v-if="requests.length" style="width: 100%" id="sidebar-multi-level-sidebar" class="top-0 left-0 z-40 p-2 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
        <div class="h-full px-3 py-4 overflow-y-auto dark:bg-gray-800 rounded-[20px] bg-white bg-clip-border shadow-3xl shadow-shadow-500">
            <ul class="space-y-2 font-medium p-0 m-0 cursor-pointer" v-if="requests">
                <li v-for="request in requests" :key="request.name+Math.random().toString(16).slice(2)">
                    <a @click="viewRequest(request)" class="flex items-center p-2 text-gray-900 rounded-[15px] bg-clip-border shadow-3xl shadow-shadow-500 dark:text-white bg-gray-100 dark:hover:bg-gray-700">
